...
const App = () => {
  ...
  const analyzeImage = async () => {
    ...
    const apiKey = '';
    const prompt =
      '이 사진에 있는 숫자를 읽어주세요. 7세그먼트 디스플레이(전자시계 숫자 모양) 또는 손글씨일 수 있습니다. 불필요한 설명 없이 오직 인식된 숫자만 응답하세요.';
    const payload = {
      contents: [
        {
          role: 'user',
    ...
    try {
      ...
      while (attempt < 5) {
        try {
          ...
          await new Promise((r) =>
            setTimeout(r, [1000, 2000, 4000, 8000, 16000][attempt - 1])
          );
        }
      }
  ...
  const handleKeypadSubmit = () => {
    if (doorLockInput === PHASE2_TARGET) {
      setCurrentPhase(3);
    } else {
      setDoorLockError(true);
      setDoorLockInput('');
    }
  };
  ...
  const handlePadlockSubmit = () => {
    if (padlockInput === PHASE3_TARGET) {
      setCurrentPhase(4);
    } else {
      setPadlockError(true);
      setPadlockInput('');
    }
  };
  ...
  const handleTouchEnd = (e: React.TouchEvent<HTMLDivElement>) => {
    ...
    if (elementAtDrop) {
      ...
      if (!targetSlotIndex && elementAtDrop.parentElement) {
        targetSlotIndex =
          elementAtDrop.parentElement.getAttribute('data-slot-index');
      }
  ...
  const submitToGoogleSheet = async () => {
    ...
    const data = {
      teamName,
      completionDate: formatDate(completionTime),
  ...
  const getSlotClassName = (index: number) => {
    ...
    if (cardInputs[index])
      return `${base} bg-fuchsia-100 text-fuchsia-800 border sm:border-2 border-fuchsia-400`;
    if (selectedWord)
      return `${base} bg-fuchsia-50/80 border sm:border-2 border-dashed border-fuchsia-400 animate-pulse`;
  ...
  const renderHeader = (
    ...
    <div className="absolute top-0 left-0 w-full p-3 sm:p-4 flex justify-between items-start z-[100] bg-white/80 backdrop-blur-md border-b border-slate-200 shadow-sm">
      <div
        className={`${theme.header} text-white px-3 sm:px-4 py-2 rounded-xl font-black text-sm sm:text-lg lg:text-xl flex items-center shadow-md animate-in slide-in-from-left-4 fade-in`}
      >
        <Flag className="w-4 h-4 sm:w-5 sm:h-5 lg:w-6 lg:h-6 mr-1.5 sm:mr-2 opacity-90 hidden sm:block" />{' '}
      ...
      <div className="flex items-center space-x-2">
        {showClock && (
          <div className="bg-white border-2 border-slate-200 rounded-xl p-2 sm:p-3 shadow-md flex items-center space-x-2 animate-in slide-in-from-right-4 fade-in">
            <Clock
              className={`w-4 h-4 sm:w-5 sm:h-5 hidden lg:block ${theme.accent}`}
            />
            <div className="flex flex-col items-end">
              ...
              <span
                className={`font-mono text-sm sm:text-lg lg:text-xl font-bold leading-none tracking-wider ${theme.accent}`}
              >
      ...
      return (
        <div className="flex-1 flex flex-col items-center justify-center p-4 pt-24 space-y-6 w-full max-w-md mx-auto h-full overflow-y-auto">
          <div className="bg-white border-4 border-amber-300 rounded-[2rem] p-6 sm:p-8 shadow-xl w-full text-center animate-in zoom-in-95 duration-500 max-h-full">
            ...
            <h2 className="text-2xl sm:text-3xl md:text-4xl font-black text-amber-600 mb-2">
              방탈출 성공!
            </h2>
            <p className="text-slate-600 font-bold mb-6 sm:mb-8 text-sm sm:text-base">
              BODY ESCAPE의 모든 관문을 통과했습니다.
            </p>

            <div className="bg-amber-50 rounded-2xl p-4 border-2 border-amber-200 inline-block w-full mb-4 sm:mb-6">
              <p className="text-xs sm:text-sm text-amber-700 font-bold mb-1">
                인증용 최종 완료 시간
              </p>
            ...
            {submitStatus !== 'success' ? (
              <div className="space-y-4 bg-slate-50 p-4 sm:p-5 rounded-2xl border-2 border-slate-200">
                <div className="text-left">
                  ...
                  <input
                    ...
                    className="w-full mt-2 bg-white border-2 border-slate-300 rounded-xl px-4 py-2 sm:py-3 text-center text-slate-800 font-bold focus:outline-none focus:border-amber-500 transition-colors"
                  />
                </div>
                <button
                  onClick={submitToGoogleSheet}
                  disabled={!teamName.trim() || submitStatus === 'submitting'}
                  className={`w-full py-3 sm:py-4 rounded-xl font-bold text-base sm:text-lg flex items-center justify-center transition-all ${
                    !teamName.trim() || submitStatus === 'submitting'
                ...
                >
                  {submitStatus === 'submitting' ? (
                    ...
                    <>
                      <Send className="w-5 h-5 sm:w-6 sm:h-6 mr-2" /> 완료 기록
                      제출하기
                    </>
                  )}
                </button>
                {submitStatus === 'error' && (
                  <p className="text-red-500 text-xs sm:text-sm font-bold mt-2 flex justify-center items-center">
                    <AlertCircle className="w-4 h-4 mr-1" /> 전송 실패.
                    네트워크를 확인하세요.
                  </p>
                )}
              </div>
              ...
              <div className="bg-emerald-50 border-2 border-emerald-300 rounded-2xl p-4 sm:p-6 flex flex-col items-center animate-in fade-in">
                ...
                <p className="font-black text-lg sm:text-xl text-emerald-700">
                  기록 제출 완료!
                </p>
                <p className="text-xs sm:text-sm text-emerald-600 font-bold mt-2 bg-white px-3 py-1 rounded-full border border-emerald-200">
                  이 화면을 선생님께 보여주세요.
                </p>
              </div>
            )}
    ...
    if (currentPhase === 4) {
      ...
      return (
        <div className="flex-1 flex flex-col items-center p-4 pt-20 sm:pt-24 space-y-4 sm:space-y-6 w-full max-w-5xl mx-auto h-full overflow-y-auto">
          {draggedWord && (
            <div
              className="fixed z-[110] px-4 py-2 bg-fuchsia-500 text-white font-black rounded-xl shadow-2xl opacity-80 pointer-events-none transform -translate-x-1/2 -translate-y-1/2"
          ...
          <div className="text-center space-y-2 sm:space-y-3 bg-white/70 backdrop-blur-sm p-4 sm:p-6 rounded-2xl sm:rounded-3xl shadow-sm border border-fuchsia-100 w-full shrink-0">
            ...
            <h2 className="text-xl sm:text-2xl md:text-3xl font-black text-fuchsia-600">
              3단계 자물쇠 해제 성공!
            </h2>
            <p className="text-slate-600 font-bold text-xs sm:text-base break-keep">
              마지막 관문입니다. 보기에 있는 단어를 터치하여 선택 후 빈칸을
              누르거나, 드래그하여 채워주세요.
            </p>
          </div>
          ...
          <div className="w-full bg-white p-3 sm:p-4 rounded-2xl shadow-sm border-2 border-fuchsia-200 flex flex-wrap justify-center gap-1.5 sm:gap-2 animate-in fade-in slide-in-from-bottom-6 shrink-0">
            {availableWords.length === 0 ? (
              <span className="text-slate-400 font-bold py-1 sm:py-2 text-sm sm:text-base">
                모든 단어를 배치했습니다!
              </span>
            ) : (
              availableWords.map((word) => (
                <div
                  ...
                  className={`px-3 sm:px-4 py-1.5 sm:py-2 rounded-lg sm:rounded-xl font-black text-sm sm:text-lg cursor-grab active:cursor-grabbing transition-all select-none touch-none ${
                    selectedWord === word
                      ? 'bg-fuchsia-500 text-white shadow-lg scale-105 ring-2 ring-fuchsia-300'
          ...
          <div className="w-full max-w-sm flex flex-col items-center pb-8 shrink-0 mt-4 sm:mt-8">
            {phase4Error && (
              <p className="text-red-500 font-black mb-2 sm:mb-4 bg-red-100 px-3 sm:px-4 py-1.5 sm:py-2 rounded-full border border-red-300 animate-pulse flex items-center text-xs sm:text-base">
            ...
            <button
              ...
              className="w-full py-3 sm:py-4 md:py-5 rounded-xl sm:rounded-2xl font-black text-lg sm:text-xl md:text-2xl bg-fuchsia-600 hover:bg-fuchsia-500 text-white shadow-xl shadow-fuchsia-600/30 active:scale-95 transition-all border-b-4 border-fuchsia-800 active:border-b-0 active:translate-y-1"
            >
              정답 제출하기
            </button>
          </div>
      ...
      return (
        <div className="flex-1 flex flex-col items-center justify-center p-4 pt-20 sm:pt-24 space-y-4 sm:space-y-8 w-full max-w-3xl mx-auto h-full overflow-y-auto">
          <div className="text-center space-y-2 sm:space-y-3 bg-white/70 backdrop-blur-sm p-4 sm:p-6 rounded-2xl sm:rounded-3xl shadow-sm border border-orange-100 w-full">
            ...
            <h2 className="text-xl sm:text-2xl md:text-3xl font-black text-orange-500">
              2단계 미션 성공!
            </h2>
            <p className="text-slate-600 font-bold text-xs sm:text-base break-keep">
              단서를 종합하여 아래의 물리 버튼 자물쇠를 해제하세요.
            </p>
          </div>
          ...
          <div className="w-full max-w-xs sm:max-w-sm mt-6 sm:mt-12 animate-in fade-in slide-in-from-bottom-8 relative pb-8">
            <div className="bg-gradient-to-b from-slate-200 to-slate-400 p-5 sm:p-8 rounded-[2rem] sm:rounded-[3rem] shadow-[0_15px_30px_rgba(0,0,0,0.2)] border-y-4 sm:border-y-8 border-x-2 sm:border-x-4 border-slate-300 relative z-10">
              ...
              <div className="text-center mb-4 sm:mb-6 mt-2 sm:mt-4 flex flex-col items-center">
                ...
                <h3 className="text-slate-700 font-black tracking-widest text-sm sm:text-xl">
                  SECURITY PADLOCK
                </h3>
              </div>
              <div
                className={`bg-slate-800 text-center py-2 sm:py-4 px-2 rounded-lg sm:rounded-xl mb-6 sm:mb-8 border-2 sm:border-[3px] shadow-[inset_0_4px_8px_rgba(0,0,0,0.5)] h-14 sm:h-20 flex flex-col justify-center transition-colors ${
                  padlockError
              ...
              >
                {padlockError ? (
                  <span className="font-mono text-xl sm:text-2xl text-red-500 font-black tracking-widest animate-pulse">
                    ERROR
                  </span>
                ) : (
                  <span className="font-mono text-2xl sm:text-4xl text-emerald-400 font-black tracking-[0.5em] text-shadow">
                    {padlockInput ? '*'.repeat(padlockInput.length) : '-'}
                  </span>
                )}
              </div>
              <div className="grid grid-cols-3 gap-3 sm:gap-4">
                ...
                <button
                  onClick={() => {
                    setPadlockInput((prev) => prev.slice(0, -1));
                    setPadlockError(false);
                  }}
                  className="bg-red-500 hover:bg-red-400 text-white rounded-full aspect-square font-black shadow-[0_4px_0_#9f1239,0_10px_15px_rgba(0,0,0,0.2)] sm:shadow-[0_8px_0_#9f1239,0_15px_20px_rgba(0,0,0,0.2)] active:shadow-none active:translate-y-[4px] sm:active:translate-y-[8px] transition-all flex items-center justify-center border sm:border-2 border-red-600"
                >
                  <Delete className="w-5 h-5 sm:w-8 sm:h-8" />
                </button>
                <button
                  onClick={() => handlePadlockClick('0')}
                  className="bg-white hover:bg-slate-50 text-slate-800 rounded-full aspect-square text-xl sm:text-3xl font-black shadow-[0_4px_0_#94a3b8,0_10px_15px_rgba(0,0,0,0.2)] sm:shadow-[0_8px_0_#94a3b8,0_15px_20px_rgba(0,0,0,0.2)] active:shadow-none active:translate-y-[4px] sm:active:translate-y-[8px] transition-all flex items-center justify-center border sm:border-2 border-slate-200"
                >
                  0
                </button>
                <button
                  onClick={handlePadlockSubmit}
                  className="bg-emerald-500 hover:bg-emerald-400 text-white rounded-full aspect-square text-xs sm:text-lg font-black shadow-[0_4px_0_#059669,0_10px_15px_rgba(0,0,0,0.2)] sm:shadow-[0_8px_0_#059669,0_15px_20px_rgba(0,0,0,0.2)] active:shadow-none active:translate-y-[4px] sm:active:translate-y-[8px] transition-all flex items-center justify-center border sm:border-2 border-emerald-600 tracking-tighter"
                >
                  OPEN
                </button>
              </div>
      ...
      return (
        <div className="flex-1 flex flex-col items-center p-4 pt-20 sm:pt-24 space-y-4 sm:space-y-6 w-full max-w-5xl mx-auto h-full overflow-y-auto">
          <div className="text-center space-y-2 sm:space-y-3 animate-in fade-in slide-in-from-bottom-4 bg-white/70 backdrop-blur-sm p-4 sm:p-6 rounded-2xl sm:rounded-3xl shadow-sm border border-teal-100 w-full shrink-0">
            ...
            <h2 className="text-xl sm:text-2xl md:text-3xl font-black text-teal-600">
              1단계 미션 성공!
            </h2>
            <p className="text-slate-600 font-bold text-xs sm:text-base break-keep">
              아래의 힌트를 해석하여 도어락 비밀번호를 풀어보세요.
            </p>
          </div>
          ...
          <div className="w-full grid grid-cols-1 md:grid-cols-2 gap-4 sm:gap-5 animate-in fade-in slide-in-from-bottom-6 shrink-0">
            <div className="bg-white p-3 sm:p-4 rounded-2xl border-2 border-teal-100 shadow-md flex flex-col">
              <div className="text-xs sm:text-sm text-teal-700 mb-2 font-black px-1 flex items-center">
                <span className="bg-teal-100 px-2 py-0.5 sm:py-1 rounded-full border border-teal-200 mr-2">
                  힌트 1
                </span>{' '}
                폐용적 및 폐용량
              </div>
          ...
          <div className="w-full max-w-[280px] sm:max-w-sm mt-4 sm:mt-8 animate-in fade-in slide-in-from-bottom-8 shrink-0 pb-8">
            <div className="bg-white p-5 sm:p-8 rounded-[1.5rem] sm:rounded-[2rem] shadow-[0_15px_40px_rgba(0,0,0,0.1)] border-t border-white border-x border-slate-100 ring-1 ring-slate-200">
              <div className="flex items-center justify-center mb-4 sm:mb-6 text-slate-400">
                ...
                <span className="font-bold tracking-[0.2em] text-xs sm:text-sm">
                  SMART DOORLOCK
                </span>
              </div>
              <div
                className={`bg-slate-900 text-center py-2 sm:py-4 px-2 rounded-xl sm:rounded-2xl mb-6 sm:mb-8 border-[3px] sm:border-[4px] shadow-inner h-14 sm:h-20 flex flex-col justify-center transition-colors ${
                  doorLockError
                    ? 'border-red-400 bg-slate-800'
                    : 'border-slate-800'
                }`}
              >
                {doorLockError ? (
                  <span className="font-mono text-lg sm:text-2xl text-red-400 font-bold tracking-widest animate-pulse">
                    ERROR
                  </span>
                ) : (
                  <span className="font-mono text-2xl sm:text-4xl text-cyan-300 font-bold tracking-[0.3em] text-shadow-sm">
                    {doorLockInput ? (
                      '*'.repeat(doorLockInput.length)
                    ) : (
                      <span className="text-slate-600 opacity-50 font-sans tracking-widest text-sm sm:text-lg">
                        INPUT
                      </span>
                    )}
                  </span>
              ...
              <div className="grid grid-cols-3 gap-2.5 sm:gap-4">
                {[1, 2, 3, 4, 5, 6, 7, 8, 9].map((num) => (
                  <button
                    ...
                    className="aspect-square bg-slate-100 hover:bg-slate-200 text-slate-700 rounded-full text-xl sm:text-2xl font-mono font-black shadow-sm border border-slate-200 active:scale-95 transition-all flex items-center justify-center"
                  >
                    {num}
                  </button>
                ))}
                <button
                  onClick={() => {
                    setDoorLockInput((prev) => prev.slice(0, -1));
                    setDoorLockError(false);
                  }}
                  className="aspect-square bg-slate-100 hover:bg-red-50 text-red-500 rounded-full shadow-sm border border-slate-200 active:scale-95 flex items-center justify-center"
                >
                  <Delete className="w-5 h-5 sm:w-6 sm:h-6" />
                </button>
                <button
                  onClick={() => handleKeypadClick('0')}
                  className="aspect-square bg-slate-100 hover:bg-slate-200 text-slate-700 rounded-full text-xl sm:text-2xl font-mono font-black shadow-sm border border-slate-200 active:scale-95 flex items-center justify-center"
                >
                  0
                </button>
                <button
                  onClick={handleKeypadSubmit}
                  className="aspect-square bg-teal-500 hover:bg-teal-400 text-white rounded-full text-2xl sm:text-3xl font-black shadow-md shadow-teal-500/30 border border-teal-400 active:scale-95 flex items-center justify-center pt-1 sm:pt-2"
                >
                  *
                </button>
              </div>
    ...
    return (
      <div className="flex-1 flex flex-col items-center justify-center p-4 pt-20 space-y-6 w-full max-w-md mx-auto h-full overflow-y-auto">
        <div className="text-center space-y-2 sm:space-y-3 bg-white/60 p-4 sm:p-6 rounded-2xl sm:rounded-3xl shadow-sm border border-blue-50 backdrop-blur-sm w-full shrink-0 mt-8">
          ...
          <h1 className="text-3xl sm:text-4xl md:text-5xl font-black tracking-tight text-blue-600 drop-shadow-sm mb-1 sm:mb-2">
            BODY ESCAPE
          </h1>
          <p className="text-slate-600 font-bold text-xs sm:text-sm md:text-base break-keep">
            완성된 숫자 코드를 사진으로 찍어 인식시키세요.
          </p>
        </div>
        ...
        <div className="bg-white rounded-[1.5rem] sm:rounded-[2rem] p-4 sm:p-6 border border-slate-200 shadow-xl space-y-4 sm:space-y-6 w-full shrink-0">
          <div className="relative w-full aspect-video sm:aspect-square max-h-[30dvh] sm:max-h-none bg-slate-50 rounded-xl sm:rounded-2xl border-2 border-dashed border-blue-300 overflow-hidden flex flex-col items-center justify-center">
            {imagePreview ? (
              <>
                ...
                {isProcessing && (
                  <div className="absolute inset-0 bg-white/80 backdrop-blur-sm flex flex-col items-center justify-center z-10">
                    <Loader2 className="w-8 h-8 sm:w-12 sm:h-12 text-blue-500 animate-spin mb-2 sm:mb-4" />
                    <p className="text-blue-600 font-bold text-xs sm:text-base animate-pulse">
                      암호 해독 중...
                    </p>
                  </div>
                )}
              ...
              <div className="text-center text-slate-400 p-4 sm:p-6 flex flex-col items-center">
                <ScanLine className="w-12 h-12 sm:w-20 sm:h-20 mb-2 sm:mb-4 text-blue-300" />
          ...
          <div className="w-full">
            ...
            <button
              ...
              className="w-full flex items-center justify-center py-3 sm:py-4 bg-slate-100 hover:bg-slate-200 text-slate-700 rounded-xl sm:rounded-2xl text-base sm:text-lg font-black transition-colors border border-slate-200 shadow-sm"
            >
          ...
          <button
            ...
            className={`w-full py-3 sm:py-5 rounded-xl sm:rounded-2xl font-black text-lg sm:text-xl flex items-center justify-center transition-all ${
        ...
        {phase1Status === 'fail' && (
          <div className="rounded-xl sm:rounded-2xl p-4 sm:p-6 border-2 bg-red-50 border-red-200 text-red-600 shadow-sm animate-in fade-in slide-in-from-bottom-4 w-full shrink-0 pb-8">
            <div className="flex flex-col items-center text-center space-y-2 sm:space-y-3">
              ...
              <p className="text-red-500/80 font-bold text-sm sm:text-base">
                인식 코드:{' '}
                <span className="font-mono text-lg sm:text-xl text-red-700 bg-red-100 px-2 py-1 rounded-md sm:rounded-lg border border-red-200 ml-1">
                  {recognizedNumber || '없음'}
                </span>
              </p>
            ...
            <button
              ...
              className="w-full mt-4 sm:mt-6 py-3 sm:py-4 rounded-lg sm:rounded-xl font-bold bg-red-500 hover:bg-red-400 text-white shadow-md shadow-red-500/20 active:scale-95 transition-all text-sm sm:text-base"
            >
              재촬영 하기
            </button>
          </div>
      ...
      <div
        className={`w-full ${
          isFullscreen
            ...
            : 'max-w-[calc(100vh*16/9)] h-[100dvh] md:h-[90vh] md:max-h-[900px] md:aspect-video md:rounded-3xl'
        } ${
          theme.bg
        } shadow-2xl overflow-hidden flex flex-col relative transition-all duration-300`}
      >
        {currentPhase > 0 &&
          renderHeader(
            currentPhase === 1
