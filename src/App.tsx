...
const App = () => {
  ...
  const getSlotClassName = (index: number) => {
    // 텍스트 크기와 상자 여백을 기기에 맞게 적절히 조절 (작아진 상자에 맞게 텍스트 조정)
    const base = "absolute flex items-center justify-center text-[10px] sm:text-xs md:text-sm font-black rounded-md cursor-pointer transition-colors z-10 px-1";
        ...
        <div className="flex-1 flex flex-col items-center p-4 pt-20 sm:pt-24 space-y-4 sm:space-y-6 w-full max-w-5xl mx-auto h-full overflow-y-auto">
          ...
          <div className="w-full grid grid-cols-1 md:grid-cols-2 gap-4 sm:gap-8 justify-items-center animate-in fade-in slide-in-from-bottom-8">
            <div className="relative w-full max-w-[280px] sm:max-w-[350px] shadow-[0_10px_30px_rgba(33,44,102,0.3)] rounded-xl overflow-hidden border-[3px] sm:border-4 border-[#212c66] bg-[#3a478b]">
               ...
               {/* 1번 이미지 (대순환) 위치 완벽 조정: 너비 19%, 높이 11% / 화살표 중앙 정렬 */}
               <div data-slot-index="0" onDrop={(e) => handleDrop(e, 0)} onDragOver={handleDragOver} onClick={() => handleSlotClick(0)} className={`${getSlotClassName(0)} top-[46%] left-[48%] w-[19%] h-[11%]`}>{cardInputs[0]}</div>
               <div data-slot-index="1" onDrop={(e) => handleDrop(e, 1)} onDragOver={handleDragOver} onClick={() => handleSlotClick(1)} className={`${getSlotClassName(1)} top-[77%] left-[60%] w-[19%] h-[11%]`}>{cardInputs[1]}</div>
            </div>
            ...
            <div className="relative w-full max-w-[280px] sm:max-w-[350px] shadow-[0_10px_30px_rgba(33,44,102,0.3)] rounded-xl overflow-hidden border-[3px] sm:border-4 border-[#212c66] bg-[#3a478b]">
               ...
               {/* 2번 이미지 (소순환) 위치 완벽 조정: 너비 19%, 높이 11% / 화살표 중앙 정렬 */}
               <div data-slot-index="2" onDrop={(e) => handleDrop(e, 2)} onDragOver={handleDragOver} onClick={() => handleSlotClick(2)} className={`${getSlotClassName(2)} top-[46%] left-[48%] w-[19%] h-[11%]`}>{cardInputs[2]}</div>
               <div data-slot-index="3" onDrop={(e) => handleDrop(e, 3)} onDragOver={handleDragOver} onClick={() => handleSlotClick(3)} className={`${getSlotClassName(3)} top-[77%] left-[30%] w-[19%] h-[11%]`}>{cardInputs[3]}</div>
               <div data-slot-index="4" onDrop={(e) => handleDrop(e, 4)} onDragOver={handleDragOver} onClick={() => handleSlotClick(4)} className={`${getSlotClassName(4)} top-[77%] left-[67%] w-[19%] h-[11%]`}>{cardInputs[4]}</div>
            </div>
          </div>
